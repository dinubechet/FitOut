...
public class PeopleFeedAdapter extends BaseAdapter {
    ...
    public void setPeopleModelList2(List<User> peopleModelList) {
        this.peopleList = peopleModelList;
        notifyDataSetChanged();
    }

    public PeopleFeedAdapter(Context context) {
        this.context = (SportActivity) context;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (position == peopleList.size() - 1 && readyToCall) {
            page++;
            ((SportActivity) context).requestPeopleFeedPage(page);
        }
        ...
        messageButton.setOnClickListener(v -> {
            context.disableClickable();
            fetch(peopleList.get(position));
            new Handler().postDelayed(() -> context.enableClickable(), 500);
        });
